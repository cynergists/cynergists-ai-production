...
namespace App\Services\Optix;

use App\Ai\Agents\Optix;
use App\Models\PortalAvailableAgent;
use App\Models\PortalTenant;
use App\Models\User;
use Illuminate\Support\Facades\Log;

class OptixAgentHandler
{
    /**
    ...
    {
        try {
            $optixAgent = new Optix(
                user: $user,
                tenant: $tenant,
                conversationHistory: $conversationHistory
            );

            $response = $optixAgent->prompt(
                prompt: $message,
            ...
            Log::error('Laravel AI error in Optix: '.$e->getMessage(), [
                'exception' => $e,
                'user_id' => $user->id,
                'tenant_id' => $tenant->id,
            ]);
