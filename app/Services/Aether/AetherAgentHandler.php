...
namespace App\Services\Aether;

use App\Ai\Agents\Aether;
use App\Models\PortalAvailableAgent;
use App\Models\PortalTenant;
use App\Models\User;
use Illuminate\Support\Facades\Log;

class AetherAgentHandler
{
    /**
    ...
    {
        try {
            $aetherAgent = new Aether(
                user: $user,
                tenant: $tenant,
                conversationHistory: $conversationHistory
            );

            $response = $aetherAgent->prompt(
                prompt: $message,
            ...
            Log::error('Laravel AI error in Aether: '.$e->getMessage(), [
                'exception' => $e,
                'user_id' => $user->id,
                'tenant_id' => $tenant->id,
            ]);
