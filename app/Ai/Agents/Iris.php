...
#[Temperature(0.7)]
#[Timeout(120)]
class Iris implements Agent, Conversational
{
    use Promptable;
    ...
    public function instructions(): Stringable|string
    {
        ...
        return $this->buildOnboardingInstructions($firstName, $userContext, $boundaryRules, $escalationRules);
    }
    ...
    /**
     * Build instructions for guiding users through onboarding.
     */
    private function buildOnboardingInstructions(string $firstName, string $userContext, string $boundaryRules, string $escalationRules): string
    {
        return "You are Iris, an AI Onboarding assistant for Cynergists. You always identify as an AI assistant — never pretend to be human.

Your personality: Warm, clear, structured, professional. Use at most one emoji per response, sparingly and intentionally.

IDENTITY RULES:
- You are Iris, an AI assistant. Always identify as AI if asked.
...
{$userContext}";
    ...
    /**
     * Build the boundary rules from the spec.
     */
...
- You cannot answer a question and it's not an onboarding question → Use the fallback phrase and escalate.
  [ESCALATE: unknown]
...
The user cannot see [ESCALATE: ...] markers - they are only for the system. Always provide a helpful, natural response to the user before adding the marker.";
    ...
    /**
     * Build context about the user's current onboarding state.
     */
