...
it('includes boundary constraints in onboarding instructions', function () {
    ...
    $agent = new Iris(user: $user, tenant: $tenant);
...
it('includes identity rules', function () {
    ...
    expect($instructions)->toContain('Iris');
});

it('includes all onboarding data fields in sequence', function () {
    $user = User::factory()->create();
    $tenant = PortalTenant::factory()->create([
        'user_id' => (string) $user->id,
        'onboarding_completed_at' => null,
...
it('tracks collected fields in user context', function () {
    $user = User::factory()->create();
    $tenant = PortalTenant::factory()->create([
        'user_id' => (string) $user->id,
        'onboarding_completed_at' => null,
...
it('always uses onboarding mode regardless of completion status', function () {
    $user = User::factory()->create();
    $completedTenant = PortalTenant::factory()->create([
        'user_id' => (string) $user->id,
        'onboarding_completed_at' => now(),
    ]);

    $agent = new Iris(user: $user, tenant: $completedTenant);
    $instructions = (string) $agent->instructions();

    // Iris is onboarding-only — no post-onboarding support mode
    expect($instructions)->toContain('CRITICAL DATA SAVING REQUIREMENT');
    expect($instructions)->toContain('COMPLETING ONBOARDING');
});
