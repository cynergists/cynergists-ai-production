...
export type ViewPreferencesTable =
...
function createDefaultPreferences(
    config: UseViewPreferencesConfig,
): ViewPreferences {
    return {
...
export function useViewPreferences(config: UseViewPreferencesConfig) {
    ...
    const [preferences, setPreferences] =
        useState<ViewPreferences>(defaultPrefs);
    ...
    const loadPreferences = useCallback(async () => {
        try {
            ...
            if (data) {
                const savedViews =
                    (data.saved_views as unknown as SavedView[]) || [];
                const defaultViewName = (data as Record<string, unknown>)
                    .default_view_name as string | null;
                const defaultView = defaultViewName
                    ? savedViews.find((v) => v.name === defaultViewName)
                    : null;
                ...
                if (defaultView) {
                    ...
                    setPreferences({
                        columnOrder:
                            data.column_order || config.defaultColumnOrder,
                        ...
                        sortColumn:
                            ...
                            'name',
                        sortDirection:
                            (data.sort_direction as 'asc' | 'desc') ||
                            config.defaultSortDirection ||
                            'asc',
                        activeFilters:
                            (data.active_filters as Record<string, string>) ||
                            {},
                        rowsPerPage: data.rows_per_page || 50,
                        savedViews,
    ...
    const savePreferences = useCallback(
        async (newPrefs: Partial<ViewPreferences>) => {
            setSaving(true);
            try {
    ...
    const setColumnWidth = useCallback(
        (column: string, width: number) => {
            savePreferences({
                columnWidths: { ...preferences.columnWidths, [column]: width },
            });
        },
    ...
    const saveView = useCallback(
        (name: string, allowOverwrite: boolean = false): boolean => {
            ...
            if (existingIndex < 0 && preferences.savedViews.length >= 3) {
                toast({
                    ...
                    description:
                        'You can save up to 3 views. Please delete an existing view first.',
                    variant: 'destructive',
                });
                return false;
            }
            ...
            const newViews =
                existingIndex >= 0
                    ? preferences.savedViews.map((v, i) =>
                          i === existingIndex ? newView : v,
                      )
    ...
    const deleteView = useCallback(
        (name: string) => {
            const newViews = preferences.savedViews.filter(
                (v) => v.name !== name,
            );
            const newActiveView =
                preferences.activeViewName === name
                    ? null
                    : preferences.activeViewName;
            const newDefaultView =
                preferences.defaultViewName === name
                    ? null
                    : preferences.defaultViewName;
            savePreferences({
                savedViews: newViews,
                activeViewName: newActiveView,
                defaultViewName: newDefaultView,
            });
            toast({
                title: 'View deleted',
        ...
        (name: string | null) => {
            ...
            if (name) {
                toast({
                    ...
                    description: `"${name}" will load automatically`,
                });
            } else {
                toast({
                    title: 'Default view cleared',
                    description: 'No view will load automatically',
                });
            }
        },
    ...
    const resetToDefault = useCallback(() => {
        ...
        toast({
            ...
            description: 'View reset to default settings',
        });
