...
export function usePartnerAccess(): PartnerAccess {
  ...
  const fetchPartnerData = async () => {
    try {
      ...
      const isSuperAdminUser = userEmail ? SUPER_ADMIN_EMAILS.includes(userEmail) : false;
      ...
      if (hasRole) {
        const { data: partnerData, error } = await supabase.rpc('get_partner_by_user_id', {
          _user_id: session.user.id
        });
  ...
  const status = partner?.partner_status as 'pending' | 'active' | 'suspended' | null;
