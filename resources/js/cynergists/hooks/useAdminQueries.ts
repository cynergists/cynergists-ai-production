...
export function useCreateNote() {
  ...
  return useMutation({
    mutationFn: (noteData: { title: string; content?: string; note_type?: string }) =>
...
export function useUpdateNote() {
  ...
  return useMutation({
    mutationFn: ({ id, ...noteData }: { id: string; title?: string; content?: string; note_type?: string }) =>
...
export function useDocumentTemplate(documentType: string) {
  return useQuery({
    ...
    queryFn: () => callAdminApi<DocumentTemplate>("get_active_template", { document_type: documentType }),
    staleTime: STALE_TIME,
    gcTime: CACHE_TIME,
  });
}
...
export function useUpdateDocumentTemplate() {
  ...
  return useMutation({
    mutationFn: ({ id, content }: { id: string; content: string }) =>
      callAdminApi<DocumentTemplate>("update_document_template", { id }, { content }),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["admin", "document-template"] });
    },
  });
}
...
export function useCreateDocumentTemplate() {
  ...
  return useMutation({
    mutationFn: (templateData: { title: string; content: string; document_type: string }) =>
      callAdminApi<DocumentTemplate>("create_document_template", undefined, templateData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "document-template"] });
    },
  });
