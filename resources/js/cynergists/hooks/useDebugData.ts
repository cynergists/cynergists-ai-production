...
export function useSetTestMode() {
  ...
  return useMutation({
    mutationFn: async (enabled: boolean) => {
      const { data, error } = await supabase.rpc('set_test_mode', { enabled });
      if (error) throw error;
      return data;
    },
    onSuccess: () => {
...
export function useWebhookEvents(filters?: { status?: string; provider?: string; limit?: number }) {
  return useQuery({
...
export function useAttributionEvents(filters?: { event_type?: string; partner_slug?: string; limit?: number }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.partner_slug) {
        query = query.ilike('partner_slug', `%${filters.partner_slug}%`);
      }
...
export function useGenerateTestEvent() {
  ...
  return useMutation({
    mutationFn: async ({ eventType, partnerSlug }: { eventType: 'attribution' | 'payment' | 'refund'; partnerSlug?: string }) => {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) throw new Error('Not authenticated');

      const response = await supabase.functions.invoke('generate-test-event', {
