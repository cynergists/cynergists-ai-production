...
export function useCalendarsList(params: UseCalendarsListParams = {}) {
    ...
    return useQuery({
        queryKey: [
            'calendars',
            { search, sortColumn, sortDirection, page, pageSize, filters },
        ],
        queryFn: async () => {
            return callAdminApi<{ calendars: Calendar[]; total: number }>(
                'get_calendars',
                undefined,
                {
                    search,
...
export function useUpdateCalendar() {
    ...
    return useMutation({
        mutationFn: async ({
            id,
            ...updates
        }: Partial<CalendarFormData> & { id: string }) => {
...
export function useCheckSlugUnique() {
    return useMutation({
        mutationFn: async ({
            ...
            return callAdminApi<{ isUnique: boolean }>(
                'check_calendar_slug',
                undefined,
                {
                    slug,
