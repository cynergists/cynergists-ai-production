...
export function usePlatformAnalytics(dateRange: DateRange | undefined) {
  ...
  const squareQuery = useQuery({
    ...
    queryFn: async (): Promise<PlatformMetrics> => {
      try {
        const { data, error } = await supabase.functions.invoke("square-analytics", {
  ...
  const aggregated = {
    ...
    totalImpressions: 
      (data.searchConsole.configured ? (data.searchConsole.primaryMetric || 0) : 0) +
    ...
    totalEngagement:
      ...
      (data.linkedin.configured ? (data.linkedin.secondaryMetric || 0) : 0) +
      (data.meta.configured ? (data.meta.secondaryMetric || 0) : 0),
    totalRevenue: data.square.configured ? (data.square.primaryMetric || 0) : 0,
    transactionCount: data.square.configured ? (data.square.secondaryMetric || 0) : 0,
  };

  const connectedCount = Object.values(data).filter((p) => p.configured).length;
