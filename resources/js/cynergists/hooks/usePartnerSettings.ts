...
export function usePartnerSettings(partnerId: string | undefined) {
  ...
  const uploadW9 = useCallback(async (file: File): Promise<boolean> => {
    if (!partnerId) return false;
  ...
  const submitPayoutMethod = useCallback(async (data: PayoutMethodData): Promise<boolean> => {
    if (!partnerId) return false;
  ...
  const toggleMFA = useCallback(async (enabled: boolean): Promise<boolean> => {
    if (!partnerId) return false;
