  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Next Billing Date</CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <Skeleton className="h-8 w-24" />
              ...
              <div className="text-3xl font-bold">
                {subscriptions?.[0]?.end_date 
                  ? format(new Date(subscriptions[0].end_date), "MMM d")
                  : "N/A"
                }
              </div>
            )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your current AI agent subscriptions</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <div className="space-y-2">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-lg border border-border text-sm">
                    <div>
                      <p className="font-medium">Invoice #{String(i).padStart(4, '0')}</p>
                      <p className="text-muted-foreground">Dec {i}, 2025</p>
                    </div>
                    <div className="flex items-center gap-4">
