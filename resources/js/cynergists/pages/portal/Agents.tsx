...
export default function PortalAgents() {
    ...
    queryFn: async () => {
      const response = await apiClient.get<{
        agents: Array<{
          ...
          created_at: string;
          subscription: { status: string; tier: string; end_date: string | null } | null;
        }>;
      }>("/api/portal/agents");
  ...
  const totalMessages = agents?.reduce((sum, agent) => sum + (agent.usage_count || 0), 0) ?? 0;
  ...
  return (
    <div className="p-8">
      <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-foreground">My AI Agents</h1>
          <p className="mt-1 text-muted-foreground">
            Manage your active lineup and jump into conversations fast.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px,1fr]">
          <Card className="h-full">
            <CardHeader>
              ...
              <CardDescription>Quick look at your portfolio.</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col gap-4">
              ...
              <div className="rounded-xl border border-border px-4 py-3">
                ...
                <p className="text-2xl font-semibold text-foreground">{totalMessages}</p>
              </div>
              <div className="rounded-xl border border-border px-4 py-3">
                <p className="text-xs font-medium text-muted-foreground">Latest agent</p>
                <p className="text-sm font-semibold text-foreground">
                  {latestAgent?.agent_name || "—"}
                </p>
                <p className="text-xs text-muted-foreground capitalize">
                  {latestAgent?.agent_type || "No activity yet"}
                </p>
              </div>
              <Button variant="outline" asChild>
                <Link href="/portal/activity">View activity</Link>
              </Button>
            </CardContent>
          </Card>
