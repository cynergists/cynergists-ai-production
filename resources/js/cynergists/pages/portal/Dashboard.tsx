  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      return apiClient.get<{
        ...
        recentAgents: Array<{ id: string; agent_name: string; usage_count: number | null }>;
      }>("/api/portal/stats");
    },
            ...
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-muted-foreground">
                  Track your agents, content, and workflow in one place.
                </p>
              </div>
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-[320px,1fr]">
          <Card className="h-full">
            <CardHeader>
              ...
              <CardDescription>Your active lineup.</CardDescription>
            </CardHeader>
          ...
          <Card className="h-full">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Upcoming tasks from your agents.</CardDescription>
              </div>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-7 gap-2 text-xs text-muted-foreground">
                {plannerDays.map((day) => (
                  <div key={day} className="text-center font-medium uppercase">
                    {day}
                  </div>
                ))}
                {plannerDays.map((day) => (
                  <div key={`${day}-block`} className="h-16 rounded-lg border border-dashed border-border bg-muted/40" />
                ))}
              </div>

              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {plannerBlocks.map((block) => (
                  <div key={`${block.day}-${block.time}`} className="rounded-xl border border-border px-4 py-3">
