...
function getDateRangeFromPreset(preset: DatePreset): {
    ...
    switch (preset) {
        ...
        case 'last_week':
            ...
            return {
                from: lastWeekStart,
                to: endOfWeek(lastWeekStart, { weekStartsOn: 0 }),
            };
        case 'last_month':
...
export default function AdminPayments() {
    ...
    useEffect(() => {
        const loadTestMode = async () => {
            ...
            if (data) {
                ...
                setTestMode(
                    data.payment_mode === 'sandbox' || data.test_mode === true,
                );
            }
        };
        loadTestMode();
    ...
    const testModeMutation = useMutation({
        ...
        onSuccess: (enabled) => {
            ...
            toast.success(
                enabled
                    ? 'Test mode enabled - all payments will use Square Sandbox'
                    : 'Live mode enabled - payments will be processed for real',
            );
        },
        onError: (error: Error) => {
        ...
        queryFn: async () => {
            ...
            const enrichedPayments =
                payments?.map((p) => ({
                    ...p,
                    client: p.client_id
                        ? clientsMap[p.client_id] || null
                        : null,
                    partner: p.partner_id
                        ? partnersMap[p.partner_id] || null
                        : null,
                })) || [];

            // Calculate summary
            const captured = enrichedPayments.filter(
                (p) => p.status === 'captured' || p.status === 'partial_refund',
            );
            const totalRevenue = captured.reduce(
                (sum, p) => sum + (p.amount || 0),
                0,
            );
            const totalRefunds = enrichedPayments.reduce(
    ...
    const syncMutation = useMutation({
        ...
        onSuccess: (data) => {
            toast.success(
                `Synced ${data.synced.inserted} new, ${data.synced.updated} updated payments`,
            );
    ...
    const filteredPayments = payments.filter((p) => {
        ...
        if (activeDateRange.from || activeDateRange.to) {
            ...
            if (activeDateRange.from && paymentDate < activeDateRange.from)
                return false;
            if (
                activeDateRange.to &&
                paymentDate > endOfDay(activeDateRange.to)
            )
                return false;
        }
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2 rounded-lg border bg-muted/50 px-3 py-1.5">
                                <FlaskConical
                                    className={`h-4 w-4 ${testMode ? 'text-amber-500' : 'text-muted-foreground'}`}
                                />
                                <Label
                                    htmlFor="test-mode"
                                ...
                                <Switch
                                    ...
                                    onCheckedChange={(checked) =>
                                        testModeMutation.mutate(checked)
                                    }
                        ...
                        <Card className="border-amber-500/50">
                            <CardContent className="flex flex-col items-center justify-center py-12">
                                ...
                                <p className="mb-4 max-w-md text-center text-sm text-muted-foreground">
                                    Click "Sync from Square" to pull your
                                    payment history, or configure Square
