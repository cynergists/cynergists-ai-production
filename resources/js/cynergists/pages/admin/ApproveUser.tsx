        ...
        try {
            ...
            if (!session) {
                ...
                sessionStorage.setItem(
                    ...
                    `/admin/approve-user?token=${token}${actionParam ? `&action=${actionParam}` : ''}`,
                );
                router.visit('/signin');
                return;
            }
            ...
            if (adminError || !adminData?.isAdmin) {
                if (adminData?.isPending) {
                    toast({
                        ...
                        description:
                            'Your admin access is still pending approval. You cannot approve other requests yet.',
                        variant: 'destructive',
                    });
                } else {
                    toast({
                        title: 'Access denied',
                        description:
                            'You must be an approved admin to process these requests.',
                        variant: 'destructive',
                    });
                }
                router.visit('/');
    ...
    const handleAction = async (action: 'approve' | 'reject') => {
        ...
        try {
            ...
            toast({
                title: 'Error',
                description:
                    'Failed to process the approval request. Please try again.',
                variant: 'destructive',
            });
            setStatus('error');
        } finally {
            ...
            case 'approved':
                return (
                    <div className="py-12 text-center">
                        ...
                        <p className="mb-6 text-sm text-muted-foreground">
                            They will receive an email notification with access
                            instructions.
                        </p>
                        <Link href="/admin/users">
                            <Button>Go to Admin Users</Button>
            ...
            case 'error':
                return (
                    <div className="py-12 text-center">
                        ...
                        <p className="mb-6 text-muted-foreground">
                            There was an error processing this request. Please
                            try again.
                        </p>
                        <Button onClick={fetchApprovalRequest}>
                            Try Again
                        </Button>
                    </div>
                );
