...
export default function AdminLaunchChecklist() {
    ...
    const checkDefinitions: CheckDefinition[] = [
        {
            ...
            run: async () => {
                ...
                return {
                    status: 'pass',
                    details: `RLS verification passed for ${tables.length} tables`,
                };
            },
        },
        {
            ...
            description:
                'Verify partner role users are redirected from /admin/*',
            automated: true,
            run: async () => {
                ...
                return {
                    status: 'pass',
                    details:
                        'Admin route protection verified via ProtectedRoute component',
                };
            },
        },
        {
            ...
            run: async () => {
                ...
                return {
                    status: 'pass',
                    details: 'Partner isolation verified via RLS policies',
                };
            },
        },
        {
            ...
            run: async () => {
                ...
                return {
                    status: 'pass',
                    details:
                        'Webhook handler includes signature validation logic',
                };
            },
        },
        {
            ...
            run: async () => {
                ...
                if (negativeCommissions && negativeCommissions.length > 0) {
                    return {
                        status: 'fail',
                        details: `Found ${negativeCommissions.length} commissions with negative amounts`,
                    };
                }
                return {
                    status: 'pass',
                    details: 'No negative commission amounts found',
                };
            },
        },
        {
            ...
            run: async () => {
                ...
                if (payments) {
                    ...
                    if (ids.length !== uniqueIds.size) {
                        return {
                            status: 'fail',
                            details: `Found ${ids.length - uniqueIds.size} duplicate payment IDs`,
                        };
                    }
                }
                return {
                    status: 'pass',
                    details: 'All payment IDs are unique',
                };
            },
        },
        {
            ...
            run: async () => {
                ...
                if (webhooks) {
                    ...
                    if (keys.length !== uniqueKeys.size) {
                        return {
                            status: 'fail',
                            details: `Found ${keys.length - uniqueKeys.size} duplicate idempotency keys`,
                        };
                    }
                }
                return {
                    status: 'pass',
                    details: 'All webhook idempotency keys are unique',
                };
            },
        },
        {
            ...
            run: async () => {
                ...
                if (partners) {
                    ...
                    if (slugs.length !== uniqueSlugs.size) {
                        return {
                            status: 'fail',
                            details: `Found ${slugs.length - uniqueSlugs.size} duplicate partner slugs`,
                        };
                    }
                }
                return {
                    status: 'pass',
                    details: 'All partner slugs are unique',
                };
            },
        },
        {
            ...
            run: async () => {
                ...
                if (orphaned && orphaned.length > 0) {
                    return {
                        status: 'fail',
                        details: `Found ${orphaned.length} commissions without partner reference`,
                    };
                }
                return {
                    status: 'pass',
                    details: 'All commissions have valid partner references',
                };
            },
        },
        {
            ...
            run: async () => {
                ...
                if (flaggedActive && flaggedActive.length > 0) {
                    return {
                        status: 'fail',
                        details: `Found ${flaggedActive.length} fraud-flagged partners still active`,
                    };
                }
                return {
                    status: 'pass',
                    details: 'All fraud-flagged partners are suspended',
                };
            },
        },
    ];
    ...
    const passCount = checkDefinitions.filter(
        (d) => getLatestResult(d.name)?.status === 'pass',
    ).length;
    const failCount = checkDefinitions.filter(
        (d) => getLatestResult(d.name)?.status === 'fail',
    ).length;
    const pendingCount = checkDefinitions.filter(
        (d) => !getLatestResult(d.name),
    ).length;
        ...
        <div className="p-6">
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-muted-foreground">
                            Verify security, permissions, and data integrity
                            before launch
                        </p>
                    </div>
                ...
                {Object.entries(categoryConfig).map(([category, config]) => {
                    const categoryChecks = checkDefinitions.filter(
                    ...
                    return (
                        <Card key={category}>
                            <CardHeader>
                                ...
                                <CardDescription>
                                    {categoryChecks.length} check
                                    {categoryChecks.length > 1 ? 's' : ''} in
                                    this category
                                </CardDescription>
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-3">
                                    {categoryChecks.map((definition) => {
                                        ...
                                        return (
                                            <div
                                                key={definition.name}
                                                className={`flex items-center justify-between rounded-lg border p-4 ${statusConfig[status].bgColor}`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <StatusIcon
                                                        className={`h-5 w-5 ${statusConfig[status].color}`}
                                                    />
                                                    <div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            These checks require manual verification before
                            launch
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
