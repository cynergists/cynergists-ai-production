...
import { Search, Loader2, UserPlus, ExternalLink, Filter, X, Save } from "lucide-react";
...
export default function Partners() {
  const { partners, loading, error, summary, refetch, createPartner, updatePartner, deletePartner, mutating } = usePartnersList();
  ...
  const processedPartners = useMemo(() => {
    ...
    if (debouncedSearch) {
      ...
      result = result.filter((partner) => {
        ...
        return (
          ...
          (partner.company_name?.toLowerCase().includes(query) ?? false)
        );
      });
    }
    ...
    // Apply status filter
    if (filters.status) {
      result = result.filter((partner) => partner.partner_status === filters.status);
    }
