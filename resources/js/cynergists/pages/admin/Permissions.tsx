...
import { useAdminPermissions, AdminUserType } from "@/hooks/useAdminPermissions";
...
export default function Permissions() {
  ...
  const isPermissionEnabled = (permissionId: string, role: AdminUserType): boolean => {
  ...
  // Toggle a permission
  const handleToggle = (permissionId: string, role: AdminUserType, enabled: boolean) => {
    const key = `${role}:${permissionId}`;
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const updates: { role: AdminUserType; permission_id: string; enabled: boolean }[] = [];
      
      pendingChanges.forEach((enabled, key) => {
        const [role, permissionId] = key.split(":") as [AdminUserType, string];
  ...
  const handleResetToDefaults = async () => {
    ...
    try {
      ...
      const upserts: { role: AdminUserType; permission_id: string; enabled: boolean }[] = [];
      const roles: AdminUserType[] = ["admin", "sales_rep", "employee"];

      for (const role of roles) {
        for (const perm of permissions) {
          const enabled = DEFAULT_PERMISSIONS[role].includes(perm.key);
      ...
      toast.error("Failed to reset permissions: " + (error as Error).message);
    } finally {
      setIsSaving(false);
    }
  };

  const isLoading = isLoadingPermissions || isLoadingRolePerms || isLoadingPerms;
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-center">Sales Rep</TableHead>
                ...
                {perms.map((perm) => (
                  <TableRow key={perm.id}>
                    ...
                    <TableCell className="text-center">
                      <Switch
                        checked={isPermissionEnabled(perm.id, "employee")}
                        onCheckedChange={(checked) =>
                          handleToggle(perm.id, "employee", checked)
                        }
                      />
                    </TableCell>
                    <TableCell className="text-center">
                      <Switch
                        checked={isPermissionEnabled(perm.id, "sales_rep")}
                        onCheckedChange={(checked) =>
                          handleToggle(perm.id, "sales_rep", checked)
                        }
                      />
                    </TableCell>
                    <TableCell className="text-center">
                      <Switch
                        checked={isPermissionEnabled(perm.id, "admin")}
                        onCheckedChange={(checked) =>
                          handleToggle(perm.id, "admin", checked)
                        }
                      />
                    </TableCell>
                  </TableRow>
