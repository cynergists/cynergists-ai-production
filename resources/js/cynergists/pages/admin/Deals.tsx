...
export default function AdminDeals() {
    ...
    const fetchDeals = async () => {
        ...
        try {
            ...
            if (dealIdParam && data) {
                const deal = (data as unknown as Deal[]).find(
                    (d) => d.id === dealIdParam,
                );
                if (deal) {
    ...
    const getPartnerName = (d: Deal) => {
        if (d.partners) {
            return (
                [d.partners.first_name, d.partners.last_name]
                    .filter(Boolean)
                    .join(' ') || d.partners.slug
            );
        }
        return null;
    };
    ...
    const openDeals = deals.filter((d) =>
        ['new', 'in_progress'].includes(d.stage),
    ).length;
                ...
                {dealsList.map((d) => (
                    ...
                    >
                        <TableCell>
                            <Badge
                                variant={
                                    stageConfig[d.stage]?.variant || 'outline'
                                }
                            >
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Manage partner deals and track progress
                    </p>
                </div>
            </div>
                        ...
                        <Tabs defaultValue="all" className="w-full">
                            <TabsList className="mb-4">
                                <TabsTrigger value="all">
                                    All ({dealsByStage.all.length})
                                </TabsTrigger>
                                <TabsTrigger value="new">
                                    New ({dealsByStage.new.length})
                                </TabsTrigger>
                                <TabsTrigger value="in_progress">
                                    In Progress (
                                    {dealsByStage.in_progress.length})
                                </TabsTrigger>
                                <TabsTrigger value="closed_won">
                                    Closed Won ({dealsByStage.closed_won.length}
                                    )
                                </TabsTrigger>
                                <TabsTrigger value="closed_lost">
                                    Closed Lost (
