...
import { ArrowLeft, Plus, Trash2, Circle, Eye, Store, Users, Handshake, Loader2 } from "lucide-react";
...
export default function AIAgentEdit({ id }: { id: string }) {
  ...
  const [previewMode, setPreviewMode] = useState<'product' | 'card'>('product');
  ...
  useEffect(() => {
    if (existingAgent) {
      ...
      const websiteCategories = Array.isArray(rawWebsiteCat) 
        ? rawWebsiteCat 
        : (typeof rawWebsiteCat === 'string' ? [rawWebsiteCat] : ["New"]);
      ...
      setFormData({
        ...
        features: Array.isArray(existingAgent.features) ? existingAgent.features.join("\n") : "",
        perfect_for: Array.isArray(existingAgent.perfect_for) ? existingAgent.perfect_for.join("\n") : "",
        integrations: Array.isArray(existingAgent.integrations) ? existingAgent.integrations.join("\n") : "",
        image_url: existingAgent.image_url || "",
        card_media: Array.isArray(existingAgent.card_media) ? existingAgent.card_media as unknown as MediaItem[] : legacyCardMedia,
        product_media: Array.isArray(existingAgent.product_media) ? existingAgent.product_media as unknown as MediaItem[] : [],
        tiers: Array.isArray(existingAgent.tiers) ? existingAgent.tiers as unknown as AgentTier[] : [],
      });
  ...
  const performSave = useCallback(async () => {
    ...
    try {
      ...
      const perfectForArray = formData.perfect_for.split("\n").map(f => f.trim()).filter(Boolean);
      const integrationsArray = formData.integrations.split("\n").map(f => f.trim()).filter(Boolean);
      ...
      const agentData = {
        ...
        price: typeof formData.price === "number" ? formData.price : 0,
        category: formData.category,
        website_category: formData.website_categories.length > 0 ? formData.website_categories : ["New"],
  ...
  const addTier = () => {
    setFormData({ ...formData, tiers: [...formData.tiers, { price: 0, description: "" }] });
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">
              {isNew ? "Create AI Agent" : `Edit: ${existingAgent?.name}`}
            </h1>
          </div>
        </div>
              ...
              <div className="space-y-6 pr-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Media shown on the agent card in listings
                  </p>
                  <AgentMediaUpload
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">Select where this agent appears</p>
                    <div className="grid grid-cols-2 gap-2 p-3 rounded-lg border bg-muted/30">
                      {WEBSITE_CATEGORIES.map((cat) => {
                        const isSelected = formData.website_categories.includes(cat);
                        return (
                          <label
                            key={cat}
                            className={`flex items-center gap-2 p-2 rounded cursor-pointer transition-colors ${
                              isSelected ? 'bg-lime-500/20 border border-lime-500/40' : 'hover:bg-muted'
                            }`}
                          >
                            <input
                              type="checkbox"
                              ...
                              onChange={(e) => {
                                const newCategories = e.target.checked
                                  ? [...formData.website_categories, cat]
                                  : formData.website_categories.filter(c => c !== cat);
                                setFormData({ ...formData, website_categories: newCategories });
                              }}
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">Used when no tiers are defined</p>
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Add tiers for slider-based pricing</p>
                    </div>
                        ...
                        <Card key={index} className="bg-muted/30">
                          <CardContent className="p-4">
                            <div className="flex items-start gap-3">
                              <Circle className="h-3 w-3 fill-lime-400 text-lime-400 mt-3 shrink-0" />
                ...
                <div className="flex justify-center py-8">
                  <AIAgentCard 
                    agent={{
                      ...
                      slug: formData.name?.toLowerCase().replace(/\s+/g, '-') || 'preview',
                      description: formData.description || null,
                      job_title: formData.job_title || null,
                      price: typeof formData.price === 'number' ? formData.price : 0,
                      category: formData.category,
