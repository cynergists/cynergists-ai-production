...
export default function AdminPayouts() {
  ...
  const fetchPayoutItems = async (payoutId: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select("id, payout_id, net_amount, earned_at, status, customer_id")
  ...
  const handleReconcile = async (payoutId: string) => {
    ...
    try {
      const { data, error } = await supabase.rpc("reconcile_payout", { p_payout_id: payoutId });
      if (error) throw error;
      
      const result = data as { removed_count?: number } | null;
      toast.success(`Reconciled: removed ${result?.removed_count || 0} commissions`);
      fetchPayouts();
  ...
  const handleExecute = async (payoutId: string) => {
    ...
    try {
      ...
      toast.info("Payout is now processing. Use 'Mark Paid' after completing transfer.");
      fetchPayouts();
    } catch (error) {
  ...
  const filteredPayouts = payouts.filter(payout => {
    const matchesSearch = search === "" || 
      payout.partner?.first_name?.toLowerCase().includes(search.toLowerCase()) ||
      payout.partner?.last_name?.toLowerCase().includes(search.toLowerCase()) ||
      payout.partner?.company_name?.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage partner payout batches and execution</p>
        </div>
                ...
                {filteredPayouts.length === 0 ? (
                  ...
                  filteredPayouts.map((payout) => (
                    ...
                    >
                      <TableCell>
                        <div>
                          ...
                          {payout.partner?.company_name && (
                            <p className="text-sm text-muted-foreground">{payout.partner.company_name}</p>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {payout.paid_at 
                          ? format(new Date(payout.paid_at), "MMM d, yyyy HH:mm")
                          : "-"}
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex gap-2 justify-end" onClick={(e) => e.stopPropagation()}>
                          {["scheduled", "ready"].includes(payout.status) && (
                            <>
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleReconcile(payout.id)}
                                disabled={actionLoading === `reconcile-${payout.id}`}
                              >
                                {actionLoading === `reconcile-${payout.id}` ? (
                              ...
                              {payout.status === "scheduled" && (
                                <Button
                                  size="sm"
                                  variant="secondary"
                                  onClick={() => handleMarkReady(payout.id)}
                                  disabled={actionLoading === `ready-${payout.id}`}
                                >
                                  {actionLoading === `ready-${payout.id}` ? (
                                    <Loader2 className="h-4 w-4 animate-spin" />
                                  ) : (
                                    "Ready"
                                  )}
                              ...
                              <Button
                                ...
                                disabled={actionLoading === `execute-${payout.id}`}
                              >
                                {actionLoading === `execute-${payout.id}` ? (
                                  <Loader2 className="h-4 w-4 animate-spin" />
                                ) : (
                          ...
                          {!["paid", "canceled"].includes(payout.status) && (
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => handleCancel(payout.id)}
                              disabled={actionLoading === `cancel-${payout.id}`}
                            >
                              {actionLoading === `cancel-${payout.id}` ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                              ) : (
          ...
          {selectedPayout && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>
                  {selectedPayout.partner?.first_name} {selectedPayout.partner?.last_name}
                  {selectedPayout.partner?.company_name && ` • ${selectedPayout.partner.company_name}`}
                </SheetDescription>
              </SheetHeader>
              ...
              <div className="mt-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-muted rounded-lg">
                    ...
                    <p className="text-2xl font-bold">{formatCurrency(selectedPayout.total_amount)}</p>
                  </div>
                  <div className="p-4 bg-muted rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span>
                      {selectedPayout.payout_date 
                        ? format(new Date(selectedPayout.payout_date), "MMM d, yyyy")
                        : "-"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>
                      {selectedPayout.partner?.payout_provider === "manual" ? "Manual" : "Direct Deposit"}
                      {selectedPayout.partner?.payout_last4 && ` (****${selectedPayout.partner.payout_last4})`}
                    </span>
                  </div>
                ...
                {/* Commission Items */}
                <div>
                  <h4 className="font-medium mb-3">Included Commissions ({payoutItems.length})</h4>
                  {loadingItems ? (
                        ...
                        <div key={item.id} className="p-3 flex justify-between items-center">
                          <div>
                            <p className="text-sm font-medium">
                              {item.commission?.earned_at 
                                ? format(new Date(item.commission.earned_at), "MMM d, yyyy")
                                : "Unknown date"}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {item.commission?.status || "Unknown status"}
                            </p>
                          </div>
