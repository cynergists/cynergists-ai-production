...
import { Loader2, Upload, FolderOpen, Bell, Shield } from "lucide-react";
...
export default function CynessaConfig({ agentId, configuration }: CynessaConfigProps) {
  ...
  const [voiceEnabled, setVoiceEnabled] = useState(currentConfig.voiceEnabled);
  const [notificationsEnabled, setNotificationsEnabled] = useState(currentConfig.notificationsEnabled);
  const [googleDriveIntegration, setGoogleDriveIntegration] = useState(currentConfig.googleDriveIntegration);
  const [crmSyncEnabled, setCrmSyncEnabled] = useState(currentConfig.crmSyncEnabled);
  const [escalationEmail, setEscalationEmail] = useState(currentConfig.escalationEmail ?? "");
  const [preferredGreeting, setPreferredGreeting] = useState(currentConfig.preferredGreeting ?? "");
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure how Cynessa guides you through the onboarding process
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch checked={crmSyncEnabled} onCheckedChange={setCrmSyncEnabled} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-sm text-muted-foreground">
              Where to send urgent matters that need human attention
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-primary/5 border-primary/20">
        ...
        <CardContent className="text-sm text-muted-foreground space-y-2">
          <p>
            Cynessa is your AI onboarding assistant, designed to make getting started with
            Cynergists as smooth as possible.
          </p>
          <p>
            She can help you upload brand assets, answer questions about our services, and
            connect you with the right team members when needed.
          </p>
          <p className="font-medium text-foreground">
            💡 Tip: Voice features are coming soon! They'll be enabled automatically when ready.
          </p>
        </CardContent>
      </Card>
    </div>
