...
const PhoneInput = React.forwardRef<HTMLInputElement, PhoneInputProps>(
    ({ className, value, onChange, ...props }, ref) => {
        ...
        const [selectedCountry, setSelectedCountry] =
            React.useState<CountryCode>(detectedCountry || COUNTRY_CODES[0]);

        // Extract local number from full phone value
        const localNumber = value
            ? extractLocalNumber(value, selectedCountry.dialCode)
            : '';
        const formattedLocal = formatPhoneNumber(
            localNumber,
        ...
        const handleCountryChange = (countryCode: string) => {
            ...
            if (country) {
                ...
                const oldDialDigits = selectedCountry.dialCode.replace(
                    /[^\d]/g,
                    '',
                );
        ...
        const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (digits.length >= 3) {
                ...
                if (
                    autoDetected &&
                    autoDetected.code !== selectedCountry.code
                ) {
