...
export function CommissionDetailDrawer({ commission, isOpen, onClose, partnerId, onUpdate }: CommissionDetailDrawerProps) {
  ...
  const handleDispute = async () => {
    ...
    try {
      ...
      toast.success("Dispute submitted. We'll review and get back to you.");
  ...
  const canDispute = ["pending", "earned", "payable", "clawed_back"].includes(commission.status);
          ...
          <ScrollArea className="flex-1 -mx-6 px-6">
            <div className="space-y-6 pb-6">
              ...
              <div className="text-center py-4 bg-muted/30 rounded-lg">
                ...
                <p className="text-3xl font-bold text-primary">{formatCurrency(commission.net_amount)}</p>
                <p className="text-xs text-muted-foreground mt-1">
                  {Math.round(commission.commission_rate * 100)}% of {formatCurrency(commission.gross_amount)}
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{commission.customer?.name || "—"}</span>
                  </div>
                  {commission.customer?.company && (
                    <div className="flex items-center gap-2">
                      <Building2 className="h-4 w-4 text-muted-foreground" />
                      <span>{commission.customer.company}</span>
                    </div>
                  )}
                  {commission.product_name && (
                    <div className="flex items-center gap-2">
                      <CreditCard className="h-4 w-4 text-muted-foreground" />
                      <span>{commission.product_name}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">Clawback Until</span>
                    <span className={commission.clawback_eligible_until && new Date(commission.clawback_eligible_until) > new Date() ? "text-yellow-600" : ""}>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">Payable At</span>
                    <span>{commission.payable_at ? format(new Date(commission.payable_at), "MMM d, yyyy") : "—"}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">Paid At</span>
                    <span className={commission.paid_at ? "text-green-600" : ""}>
                      {commission.paid_at ? format(new Date(commission.paid_at), "MMM d, yyyy") : "—"}
                    </span>
                  </div>
                </div>
              </div>
      ...
      <Dialog open={disputeOpen} onOpenChange={setDisputeOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Explain why you believe this commission is incorrect. We'll review and respond within 2 business days.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 pt-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleDispute} disabled={submitting || !disputeReason}>
