...
import { User, Building2, Phone, Mail, Calendar, DollarSign, Clock, Ticket, ExternalLink } from "lucide-react";
...
export function DealDetailDrawer({ deal, isOpen, onClose }: DealDetailDrawerProps) {
  ...
  const handleCreateTicket = () => {
    const subject = encodeURIComponent(`Re: ${deal?.client_company || deal?.client_name || 'Deal'}`);
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span>{deal.deal_value ? formatCurrency(deal.deal_value) : 'TBD'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-muted-foreground" />
                  <span>{deal.expected_close_date ? format(new Date(deal.expected_close_date), 'MMM d, yyyy') : 'TBD'}</span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div key={note.id} className="border rounded-lg p-3 text-sm">
                    <div className="flex items-center gap-2 mb-1">
                      <Badge variant="outline" className="text-xs">
                        {note.note_type === 'partner_visible' ? 'Update' : 'System'}
                      </Badge>
                ...
                {referrals.map(ref => (
                  <div key={ref.id} className="border rounded-lg p-3 text-sm bg-muted/30">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge variant={ref.status === 'accepted' ? 'secondary' : ref.status === 'rejected' ? 'destructive' : 'outline'} className="text-xs">
                        {ref.status}
                      </Badge>
                      <span className="text-muted-foreground text-xs">
                    ...
                    <p className="text-muted-foreground">
                      {ref.lead_name || ref.lead_email} via {ref.event_type || ref.source || 'direct'}
                    </p>
                  </div>
                ))}
