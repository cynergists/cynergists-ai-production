...
export function ProspectsMetricsSummary({ summary, loading, hasActiveFilters }: ProspectsMetricsSummaryProps) {
  ...
  const metrics: {
  ...
  }[] = [
    {
      ...
      rows: [
        ...
        { label: "Committed", value: `${summary?.hotCount || 0} (${formatCurrency(summary?.hotValue || 0)})` },
        { label: "Interested", value: `${summary?.warmCount || 0} (${formatCurrency(summary?.warmValue || 0)})` },
        { label: "Aware", value: `${summary?.coldCount || 0} (${formatCurrency(summary?.coldValue || 0)})` },
      ],
      icon: Users,
    ...
    ...([0, 1, 2].map((index) => {
      ...
      return {
        ...
        rows: [
          ...
          { label: "Committed", value: `${data?.hotCount || 0} (${formatCurrency(data?.hotValue || 0)})` },
          { label: "Interested", value: `${data?.warmCount || 0} (${formatCurrency(data?.warmValue || 0)})` },
          { label: "Aware", value: `${data?.coldCount || 0} (${formatCurrency(data?.coldValue || 0)})` },
        ],
