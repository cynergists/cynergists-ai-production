...
import { AlertTriangle, Copy, Loader2, MessageSquare, ExternalLink, User, Building2, Phone, Mail, Calendar, DollarSign, Clock, Trash2 } from "lucide-react";
...
export function DealDrawer({ deal, isOpen, onClose, onUpdate }: DealDrawerProps) {
  ...
  const fetchReferrals = async () => {
    ...
    const { data } = await supabase
      .from('referrals')
      .select('id, lead_name, lead_email, source, event_type, duplicate, status, created_at')
      .eq('deal_id', deal.id)
      .order('created_at', { ascending: false });
    setReferrals(data || []);
  };
  ...
  // Auto-save handler
  const handleAutoSave = useCallback(async (data: Partial<Deal>): Promise<boolean> => {
  ...
  const confirmPartnerChange = async () => {
    ...
    try {
      ...
      const oldName = oldPartner ? [oldPartner.first_name, oldPartner.last_name].filter(Boolean).join(' ') : 'None';
      const newName = newPartner ? [newPartner.first_name, newPartner.last_name].filter(Boolean).join(' ') : 'None';

      // Update partner_id
      const { error: updateError } = await supabase
        .from('partner_deals')
        .update({ partner_id: pendingPartnerId, last_activity_at: new Date().toISOString() })
        .eq('id', deal.id);
      ...
      await supabase.from('partner_audit_logs').insert({
        ...
        new_value: { partner_id: pendingPartnerId, partner_name: newName }
      });

      setEditedDeal(prev => ({ ...prev, partner_id: pendingPartnerId }));
  ...
  const getPartnerName = (partnerId: string | null) => {
    ...
    return [partner.first_name, partner.last_name].filter(Boolean).join(' ') || partner.slug;
  };
  ...
  return (
    <>
          ...
          <ScrollArea className="flex-1 -mx-6 px-6">
            <div className="space-y-6 pb-6">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  {deal.client_email && (
                    <div className="flex items-center gap-2">
                      ...
                      <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => copyToClipboard(deal.client_email!)}>
              ...
              <div className="space-y-4">
                <h4 className="font-medium text-sm text-muted-foreground">Deal Details</h4>
                
                <div className="space-y-2">
                  <Label>Stage</Label>
                  <Select 
                    value={editedDeal.stage || deal.stage} 
                    onValueChange={(v) => setEditedDeal(prev => ({ ...prev, stage: v }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    type="number"
                    value={editedDeal.deal_value ?? deal.deal_value ?? ''}
                    onChange={(e) => setEditedDeal(prev => ({ ...prev, deal_value: parseFloat(e.target.value) || 0 }))}
                    placeholder="0"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    type="date"
                    value={editedDeal.expected_close_date?.split('T')[0] || deal.expected_close_date?.split('T')[0] || ''}
                    onChange={(e) => setEditedDeal(prev => ({ ...prev, expected_close_date: e.target.value || null }))}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    value={editedDeal.partner_id || deal.partner_id || "unassigned"} 
              ...
              {/* Add Note */}
              <div className="space-y-3">
                <h4 className="font-medium text-sm text-muted-foreground">Add Note</h4>
                <div className="space-y-2">
                  <Select value={noteType} onValueChange={setNoteType}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="partner_visible">Partner Visible</SelectItem>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  {notes.map(note => (
                    <div key={note.id} className="border rounded-lg p-3 text-sm">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge variant={note.note_type === 'admin' ? 'destructive' : note.note_type === 'partner_visible' ? 'secondary' : 'outline'} className="text-xs">
                          {note.note_type}
                        </Badge>
                  ...
                  {/* Referrals */}
                  {referrals.map(ref => (
                    <div key={ref.id} className="border rounded-lg p-3 text-sm bg-muted/30">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge variant="outline" className="text-xs">Referral</Badge>
                        {ref.duplicate && <Badge variant="outline" className="text-xs bg-orange-500/10">Duplicate</Badge>}
                        <span className="text-muted-foreground text-xs">
                          {formatDistanceToNow(new Date(ref.created_at), { addSuffix: true })}
                        </span>
                      </div>
                      <p className="text-muted-foreground">
                        {ref.lead_name || ref.lead_email} - {ref.event_type || ref.source || 'Unknown source'}
                      </p>
                    </div>
                  ))}

                  {/* Deal Timeline */}
                  {deal.timeline && Array.isArray(deal.timeline) && deal.timeline.map((entry, idx) => (
                    <div key={idx} className="border rounded-lg p-3 text-sm bg-muted/10">
                      <div className="flex items-center gap-2 mb-1">
                  ...
                  {notes.length === 0 && referrals.length === 0 && (!deal.timeline || deal.timeline.length === 0) && (
                    <p className="text-muted-foreground text-sm text-center py-4">No timeline entries yet</p>
                  )}
                </div>
              </div>
            </div>
      ...
      <Dialog open={showPartnerWarning} onOpenChange={setShowPartnerWarning}>
        <DialogContent>
          <DialogHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Changing partner attribution will affect commissions for this deal. Are you sure you want to change from <strong>{getPartnerName(oldPartnerId)}</strong> to <strong>{getPartnerName(pendingPartnerId)}</strong>?
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowPartnerWarning(false); setPendingPartnerId(null); }}>
              Cancel
            </Button>
            <Button onClick={confirmPartnerChange} disabled={isSaving}>
