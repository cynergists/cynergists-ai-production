...
import { 
  ...
  DollarSign, Clock, Loader2, Tag, Target, FileText, Briefcase, UserPlus, Trash2
} from "lucide-react";
...
const STATUS_OPTIONS: Record<ContactType, { value: ContactStatus; label: string }[]> = {
  prospect: [
...
export function UnifiedContactCard({ 
  ...
  const [status, setStatus] = useState<ContactStatus>(DEFAULT_STATUS[contactType]);
  const [notes, setNotes] = useState("");
  
  // Dialog state for termination warning
  const [showTerminationDialog, setShowTerminationDialog] = useState(false);
  const [pendingTerminationStatus, setPendingTerminationStatus] = useState<ContactStatus | null>(null);
  ...
  useEffect(() => {
    if (contact) {
      ...
      setEstClosingDate(contact.estClosingDate ? format(new Date(contact.estClosingDate), "yyyy-MM-dd") : "");
      setLastMeeting(contact.lastMeeting ? format(new Date(contact.lastMeeting), "yyyy-MM-dd") : "");
      setLastOutreach(contact.lastOutreach ? format(new Date(contact.lastOutreach), "yyyy-MM-dd") : "");
      setNextOutreach(contact.nextOutreach ? format(new Date(contact.nextOutreach), "yyyy-MM-dd") : "");
      setNextMeeting(contact.nextMeeting ? format(new Date(contact.nextMeeting), "yyyy-MM-dd'T'HH:mm") : "");
    } else {
  ...
  const buildFormData = useCallback((): ContactFormData => {
    ...
    if (contactType === 'partner') {
      ...
      const computedName = [firstName, lastName].filter(Boolean).join(' ') || name;
    ...
    if (contactType === 'prospect') {
      ...
      formData.estimatedValue = estimatedValue ? parseFloat(estimatedValue) : undefined;
    ...
    return formData;
  }, [name, firstName, lastName, nickName, email, phone, company, title, salesRep, partnerName, tags, status, notes, services, estimatedValue, leadSource, sdrSet, estClosingDate, lastMeeting, lastOutreach, nextOutreach, nextMeeting, contactType]);
  ...
  const handleStatusChange = (newStatus: ContactStatus) => {
    if (contactType === 'client' && newStatus === 'terminated' && status !== 'terminated') {
  ...
  const SourceBadge = ({ metadata }: { metadata?: FieldMetadata }) => {
    ...
    const label = source === "website" ? "Website" : source === "admin" ? "Admin" : source === "ghl" ? "GHL" : source === "square" ? "Square" : source;
    const variant = source === "website" ? "default" : source === "admin" || source === "square" ? "secondary" : "outline";
    return (
  ...
  return (
    <div className="mt-4 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                {contact && <SourceBadge metadata={contact.nameMetadata} />}
              </div>
              <Input
                id="firstName"
                value={firstName}
          ...
          <div className="space-y-1">
            ...
            />
            <p className="text-xs text-muted-foreground">If set, this name will be used in emails and communications</p>
          </div>

          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <Label htmlFor="email">Email {isCreating && contactType === 'partner' && '*'}</Label>
              {contact && <SourceBadge metadata={contact.emailMetadata} />}
            </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              {contact && <SourceBadge metadata={contact.phoneMetadata} />}
            </div>
            <div className="relative">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              {contact && <SourceBadge metadata={contact.companyMetadata} />}
            </div>
            <div className="relative">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <Select value={status} onValueChange={(v) => setStatus(v as ContactStatus)}>
                  ...
                  <SelectContent>
                    {STATUS_OPTIONS[contactType].map((opt) => (
              ...
              <div className="space-y-1">
                <Label htmlFor="estimatedValue">Est Amount</Label>
                <div className="relative">
              ...
              <div className="space-y-1">
                <Label htmlFor="estClosingDate">Est Closing Date</Label>
                <Input
                  id="estClosingDate"
                  type="date"
                  value={estClosingDate}
                  onChange={(e) => setEstClosingDate(e.target.value)}
                />
              </div>
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  {contact && <SourceBadge metadata={contact.salesRepMetadata} />}
                </div>
                <Input
                  id="salesRep"
                  value={salesRep}
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  {contact && <SourceBadge metadata={contact.partnerNameMetadata} />}
                </div>
                <Input
                  id="partnerName"
                  value={partnerName}
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  {contact && <SourceBadge metadata={contact.tagsMetadata} />}
                </div>
                <div className="relative">
        ...
        <>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label htmlFor="lastMeeting">Last Meeting</Label>
                <Input
                  id="lastMeeting"
                  type="date"
                  value={lastMeeting}
              ...
              <div className="space-y-1">
                <Label htmlFor="lastOutreach">Last Outreach</Label>
                <Input
                  id="lastOutreach"
                  type="date"
                  value={lastOutreach}
              ...
              <div className="space-y-1">
                <Label htmlFor="nextOutreach">Next Outreach</Label>
                <Input
                  id="nextOutreach"
                  type="date"
                  value={nextOutreach}
                  onChange={(e) => setNextOutreach(e.target.value)}
                />
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{contact.paymentType || "—"}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Amount</p>
                <p className="font-medium">
                  {contact.paymentAmount ? `$${contact.paymentAmount.toLocaleString()}` : "—"}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{contact.squarePlanName || "—"}</p>
              </div>
              <div>
      ...
      {/* Action Buttons */}
      <div className="sticky bottom-0 pt-4 bg-background border-t space-y-2">
            ...
            <>
              {onDelete && (
                <Button 
                  ...
                  className="flex-1"
                  onClick={onDelete} 
                  disabled={saving || deleting || isAutoSaving}
                >
                  {deleting ? (
                    <>
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                Are you sure you want to terminate this client? This action will:
              </p>
              <ul className="list-disc list-inside space-y-1 text-sm">
                <li>Cancel their active subscription in Square</li>
                <li>Stop all future automatic payments</li>
                <li>Mark the client as terminated in the system</li>
              </ul>
              <p className="font-medium text-destructive">
                This will immediately stop payments from being pulled from their bank or card.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={cancelTermination}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmTermination} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
