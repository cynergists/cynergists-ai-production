...
export function ClientsMetricsSummary({
    summary,
    loading,
    hasActiveFilters,
}: ClientsMetricsSummaryProps) {
    ...
    const totalActiveCount =
        (summary?.activeMonthlyCount ?? 0) + (summary?.activeAnnualCount ?? 0);
    const annualizedTotal = (summary?.totalActiveAmount ?? 0) * 12;

    const metrics = [
        {
            label: 'Active Accounts',
            dollarAmount: summary
            ...
            rows: [
                ...
                {
                    label: 'Terminated',
                    value: summary?.terminatedTotalCount ?? 0,
                },
                {
                    label: 'Retention',
                    value: formatPercentLocal(
                        summary?.retentionTotalPercent ?? 0,
                    ),
                },
            ],
            icon: DollarSign,
        ...
        },
        {
            ...
            dollarAmount: summary
                ? formatCurrency(summary.activeMonthlyAmount)
                : '$0',
            annualAmount: formatCurrency(
                (summary?.activeMonthlyAmount ?? 0) * 12,
            ),
            showPerMonth: true,
            rows: [
                {
                    label: 'Active Clients',
                    value: summary?.activeMonthlyCount ?? 0,
                },
                {
                    label: 'Terminated',
                    value: summary?.terminatedMonthlyCount ?? 0,
                },
                {
                    label: 'Retention',
                    value: formatPercentLocal(
                        summary?.retentionMonthlyPercent ?? 0,
                    ),
                },
            ],
            icon: Calendar,
        ...
        },
        {
            ...
            dollarAmount: summary
                ? formatCurrency(summary.activeAnnualAmount ?? 0)
                : '$0',
            annualAmount: formatCurrency(
                (summary?.activeAnnualAmount ?? 0) * 12,
            ),
            showPerMonth: true,
            rows: [
                {
                    label: 'Active Clients',
                    value: summary?.activeAnnualCount ?? 0,
                },
                {
                    label: 'Terminated',
                    value: summary?.terminatedAnnualCount ?? 0,
                },
                {
                    label: 'Retention',
                    value: formatPercentLocal(
                        summary?.retentionAnnualPercent ?? 0,
                    ),
                },
            ],
            icon: CalendarDays,
        ...
        },
        {
            ...
            dollarAmount: summary
                ? formatCurrency(summary.activeOneTimeAmount)
                : '$0',
            annualAmount: null,
            showPerMonth: false,
            rows: [
