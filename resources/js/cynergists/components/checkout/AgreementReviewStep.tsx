...
const AgreementReviewStep = ({
    ...
    const defaultInitials =
        `${contactData.firstName.charAt(0)}${contactData.lastName.charAt(0)}`.toUpperCase();
...
{{CLIENT_INITIALS}}
    ...
    const handleSigningComplete = (result: SigningResult) => {
        ...
        Object.entries(result.sectionInitials).forEach(
            ([key, value], index) => {
                const legacyKey =
                    `section${index + 3}Initials` as keyof SignatureData;
    ...
    if (loadingTemplate) {
        return (
            <div className="card-glass flex min-h-[400px] items-center justify-center p-8">
