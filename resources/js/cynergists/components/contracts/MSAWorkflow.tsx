...
import { VersionHistorySection, type TemplateVersion } from "./VersionHistorySection";
...
export default function MSAWorkflow({ 
  ...
  const [lastVersionUpdate, setLastVersionUpdate] = useState<Date | null>(null);
  ...
  const loadVersionHistory = async () => {
    ...
    try {
      const data = await callApi<TemplateVersion[]>("get_template_versions", { template_id: template.id });
      setVersions(data || []);
    } catch (error) {
  ...
  const handleCreateVersion = async (data: { changeSummary: string; effectiveDate: string }) => {
    if (!template?.id) return;
    ...
    try {
      ...
      const newVersion = await callApi<TemplateVersion>("create_new_template_version", 
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <div className="bg-card border-b px-6 py-4 shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <h2 className="text-lg font-semibold">{getDocumentTitle(template.document_type)}</h2>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              {isSaving ? (
                ...
                <Badge variant="outline" className="text-green-600 border-green-300 gap-1.5">
                  <Check className="h-3 w-3" />
                  Last Saved {lastSaved ? format(lastSaved, "M/d/yy h:mm a") : (template.updated_at ? format(new Date(template.updated_at), "M/d/yy h:mm a") : "N/A")}
                </Badge>
              )}
            </div>
            ...
            {template.id && (
              <Button 
                ...
                disabled={isUpdatingVersion || isLoading || hasUnsavedChanges}
      ...
      <div className="flex-1 min-h-0 overflow-hidden bg-white dark:bg-background flex flex-col">
        {/* Non-editable Document Header */}
        <div className="bg-muted border-b px-6 py-3 text-sm text-muted-foreground shrink-0">
          <div>
            Version: {formatVersion(currentVersion)}
          </div>
          <div>
            Version Update: {lastVersionUpdate ? format(lastVersionUpdate, "M/d/yy") : (template.updated_at ? format(new Date(template.updated_at), "M/d/yy") : "N/A")}
          </div>
        </div>
      ...
      <CreateVersionModal
        ...
        documentType={template.document_type as "msa" | "terms" | "privacy" | "partner" | "sales_reps"}
      />
    </div>
  );
}
