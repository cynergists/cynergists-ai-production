 ...
 */
export function extractInitialsRequirements(
...
export function getSigningProgress(
    ...
    const completedInitials = sectionKeys.filter(
        (key) => !!sectionInitials[key],
    ).length;
    const allInitialsComplete =
        completedInitials === requiredInitials && requiredInitials > 0;
